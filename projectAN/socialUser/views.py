...
def google_callback(request):
    ...
    try:
        access_token = __get_access_token(code)
    except JSONDecodeError:
        raise JSONDecodeError
        
    email = __get_email_address(access_token)
    if email is None:
    ...
    try:
        ...
        # 기존에 Google로 가입된 유저
        accept_json = __signIn_or_signUp(access_token, code)
...
def __signIn_or_signUp(access_token, code):
...
def __get_email_address(access_token):
